...
exports.order_add = (req, res) => {
  ...
  // );
  // console.log("inside ");
  kafka.make_request("order-topic", "ADD", req.body, function (err, results) {
    if (err) {
      ...
      res.json({ status: "ok", message: "order added" });
      res.end();
    }
  });
};
...
exports.order_get = (req, res) => {
  ...
  //   console.log(error);
  // })
  kafka.make_request("order-topic", "GET", req.query, function (err, results) {
    if (err) {
      console.log("Inside err");
      res.json({
        status: "error",
      ...
      console.log("Inside else");
      res.json({ status: "ok", ORDER_HISTORY: results });
      res.end();
    }
  });
};
